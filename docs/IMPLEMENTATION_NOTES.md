...
这是 Triton custom backend 的入口文件。主要实现 Triton backend API：
...
4. backend 收集 Triton request input buffers。
5. 调用 `BpuRuntime::Infer`。
6. 把输出 raw bytes 写入 Triton response output buffers。
7. Triton 通过 HTTP/gRPC 返回给客户端。

代码中额外做了安全检查：

- 仅接受 CPU input buffer。
- 检查 input buffer size，防止 overflow。
...
- 转换 dtype、shape、byte size、stride 等信息。
...
- 把 Triton request 中的 raw bytes 拷贝到 DNN input tensor。
- 调用 BPU runtime 同步推理。
- 把输出 tensor raw bytes 拷贝回 backend output buffer。
...
- 打印 `.hbm` metadata。
- 使用 raw input 文件跑 BPU 推理。
- dump output 文件。

这有助于区分问题来自 BPU runtime 还是 Triton backend 集成。
...
通用 Triton HTTP binary tensor client。

职责：

- 构造 Triton binary tensor request。
- 使用 `urllib.request` 发送 HTTP inference。
- 解析 Triton response header 和 raw output bytes。
...
- 读取 `hrt_model_exec model_info` 中的 output scale metadata。
- Python 侧完成 YOLO decode、NMS、annotation。
