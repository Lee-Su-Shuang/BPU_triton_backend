...
该后端可以加载 Horizon/D-Robotics `.hbm` 模型，通过板载 DNN/HBRT runtime 调用 S100P BPU 执行推理，并通过 Triton 标准 HTTP inference API 对外提供服务。

同时，本项目提供了一个实时验证工具链：通过 USB 摄像头采集画面，将画面预处理为 YOLOv5x `.hbm` 模型所需的 NV12 split-plane 输入，通过 Triton HTTP 调用 `yolov5x_bpu` 模型，再把检测结果绘制到网页 MJPEG 实时画面中。
...
- 打印模型 metadata。
- 使用原始输入文件执行本地 BPU 推理。
- 便于定位问题来自 backend / Triton 层还是 BPU runtime 层。
...
- `start_bpu_triton.sh`：编译 backend、安装 `libtriton_bpu.so`、准备模型仓库、启动 Triton。
- `infer_yolov5x_http.py`：构造 Triton binary tensor HTTP request，验证 YOLOv5x 原始输出张量。
- `stop_bpu_triton.sh`：停止 Triton。
- `build_no_gpu_stubs.sh`：为当前无 NVIDIA GPU 的板端环境准备 Triton Server 所需的 CUDA / DCGM stub。
...
- 转换 BGR frame 为 YOLOv5x 所需的 NV12 split-plane 输入。
- 使用 Triton HTTP 二进制 tensor API 调用 `yolov5x_bpu`。
- 通过 Python YOLOv5x adapter 解析输出并绘制检测框。
...
本项目已完成 RDK S100P BPU Triton backend 开发任务：
